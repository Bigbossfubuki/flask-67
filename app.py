...
app.config['SECRET_KEY']='InTheNameofGoddd'
...
def store():
    # from flask import flask, render_template, request
        if request.method == "POST":
            ...
            # session ไม่ค่อยมีคนใช้
            return redirect('/')
        else:
            session['alert_status'] = "fail"
            session['alert_message'] = "Something went wrog!"
            return redirect('/')
...
def index():
        ...
        return render_template("index.html", alert_message = alert_message, result = result)
